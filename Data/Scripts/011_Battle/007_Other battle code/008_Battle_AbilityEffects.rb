...
Battle::AbilityEffects::StatusImmunity.copy(:INSOMNIA, :SWEETVEIL, :VITALSPIRIT, :HIGHALERT)
...
Battle::AbilityEffects::MoveBlocking.add(:ANIMALBOND,
  proc { |ability, bearer, user, targets, move, battle|
    next false if !bearer.opposes?(user)
    ret = false
...
Battle::AbilityEffects::MoveImmunity.add(:WAVERIDER,
  proc { |ability, user, target, move, type, battle, show_message|
    # Increase both Speed and Special Attack
    target.pbMoveImmunityStatRaisingAbility(user, move, type,
       :WATER, :SPECIAL_ATTACK, 1, show_message)
    target.pbMoveImmunityStatRaisingAbility(user, move, type,
       :WATER, :SPEED, 1, show_message)
  }
)
...
#Battle::AbilityEffects::MoveImmunity.copy(:WATERABSORB, :WAVERIDER)
...
Battle::AbilityEffects::AccuracyCalcFromTarget.copy(:STORMDRAIN, :WAVERIDER)
...
Battle::AbilityEffects::DamageCalcFromUser.add(:CREATIVEBURST,
  proc { |ability, user, target, move, mults, power, type|
    mults[:power_multiplier] *= 1.3 if move.addlEffect > 0
  }
)
...
Battle::AbilityEffects::EndOfRoundHealing.add(:INJURYPRONE,
  proc { |ability, battler, battle|
    next if battler.status == :NONE
    next unless battle.pbRandom(100) < 30
    battle.pbShowAbilitySplash(battler)
    oldStatus = battler.status
    #battler.pbCureStatus(Battle::Scene::USE_ABILITY_SPLASH)
    battler.pbCureStatus(showMessages = false)
    
    battle.pbDisplay(_INTL("{1}'s recovery dip cured its status problem!", battler.pbThis))
    ...
    #  case oldStatus
     ...
     #   battle.pbDisplay(_INTL("{1}'s {2} woke it up!", battler.pbThis, battler.abilityName))
     # when :POISON
     #   battle.pbDisplay(_INTL("{1}'s recovery dip cured its poison!", battler.pbThis))
     # when :BURN
     #   battle.pbDisplay(_INTL("{1}'s recovery dip healed its burn!", battler.pbThis))
     # when :PARALYSIS
     #   battle.pbDisplay(_INTL("{1}'s recovery dip cured its paralysis!", battler.pbThis))
      #when :FROZEN
      #  battle.pbDisplay(_INTL("{1}'s {2} defrosted it!", battler.pbThis, battler.abilityName))
     # end
    #end
...
Battle::AbilityEffects::EndOfRoundEffect.add(:INCHARACTER,
  proc { |ability, battler, battle|
    randomUp = []
    #randomDown = []
    if Settings::MECHANICS_GENERATION >= 8
...
Battle::AbilityEffects::OnSwitchIn.add(:ENDURANCE,
  proc { |ability, battler, battle, switch_in|
    #battle.pbShowAbilitySplash(battler)
