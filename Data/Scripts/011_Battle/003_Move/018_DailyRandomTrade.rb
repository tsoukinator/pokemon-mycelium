...
def pbDailyRandomTrade
  # Define the game variable number for storing the last used date
  #DAILY_RANDOM_TRADE = 223
  
  ## Set today's random set
  daily_trade_setlist = ["Set A", "Set B", "Set C"]
  today_set = daily_trade_setlist.sample
  
  pbMessage(_INTL("Today's random selection are: {1}", today_set.to_s))
  # Player chooses Pokemon, index and name are set to game variables 1 and 3  
  pbChoosePokemon(1,3)

  # Pokemon is randomised
  random_list = [:SQUIRTLE, :CHARMANDER, :BULBASAUR]
  
  selected_pkmn = random_list.sample
  poke = Pokemon.new(selected_pkmn, 10)
...
DAILY_RANDOM_TRADE = 223 # Adjust this ID as needed
...
def pbGPTDailyTrade
  ...
  last_used_date = $game_variables[DAILY_RANDOM_TRADE]
  
  pbMessage(_INTL("Hey there, welcome to your random trade."))
  puts DAILY_RANDOM_TRADE
  ...
  if chosen_index >= 0
    ...
    player_pkmn = $player.party[pbGet(1)] # Get's pokemon from var 1
