...
#	OnInflictingStatus.trigger(:POISONPUPPETEER, user, battler, status) if user && user.hasActiveAbility?(:POISONPUPPETEER) #user.hasActiveAbility?(:POISONPUPPETEER) # Poison Puppeteer user.ability
...
if PluginManager.installed?("Generation 9 Pack")
	...
	def self.triggerOnOpposingStatGain(ability, battler, battle, statUps)
		for i in battler.abilityMutationList
		$aamName=GameData::Ability.get(i).name
		OnOpposingStatGain.trigger(i, battler, battle, statUps)
		end
	end
	...
	def self.triggerModifyTypeEffectiveness(ability, user, target, move, battle, effectiveness)
		...
		$aamName=GameData::Ability.get(i).name
		ret = trigger(ModifyTypeEffectiveness, i, user, target, move, battle, effectiveness, ret: effectiveness)	#return
		effectiveness = ret
		end
...
#  end
  ...
  if PluginManager.installed?("Generation 10 Pack")
